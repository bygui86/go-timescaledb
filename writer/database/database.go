...
const (
	...
	// error messages
	alreadyExistsErrorMsg     = "already exists"
	alreadyHypertableErrorMsg = "already a hypertable"
)
...
func New() (*sql.DB, error) {
	...
	_, dbErr := db.Exec(fmt.Sprintf(createDbQuery, cfg.dbName))
	if dbErr != nil {

		if !strings.Contains(dbErr.Error(), alreadyExistsErrorMsg) {
			return nil, dbErr
		} else {
			logging.Log.Warn("Database already exists")
		}
	}
	...
	_, hypertableErr := db.Exec(createHypertableQuery)
	if hypertableErr != nil {
		if !strings.Contains(hypertableErr.Error(), alreadyHypertableErrorMsg) {
			return nil, hypertableErr
		} else {
			logging.Log.Warn("Table already a hypertable")
		}
	}
...
func NewWithWrappedTracing() (*sql.DB, error) {
	...
	if hypertableErr != nil {
		if !strings.Contains(hypertableErr.Error(), alreadyHypertableErrorMsg) {
			...
			logging.Log.Warn("Table is already a hypertable")
		}
	}
